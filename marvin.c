...
void AksenMain(void)
{
	...
	while(1){
		if(dip_pin(1)){
			...
			//
			////////////////////////////////
			//Entfernungsmessung und abweichmanöver
			if(analog(PORT_SHARP_L) > TURNDISTANCE){
				trn_c(5);
				if(i_have_the_ball && !i_have_the_goal ){
					//Torsuche				
					// wenn linker sensor das Tor sieht
					// Um Norden herum drehen + Ball behalten
					if(ir_goal_detected_l == TRUE){
						trn_c_n(6);
					// wenn rechter sensor das Tor sieht
					// Um Norden herum drehen + Ball behalten
					}else if(ir_goal_detected_r == TRUE){
						trn_cc_n(6);
					}else{
					// drehen falls das Tor nicht sichtbar
						dir_nw(10);
						sleep(500);
					}//if
					sleep(8);
			...
			//Ball weit vorn -> geradeaus fahrensleep(10);
			}else if((analog(PORT_BALL_DETECTOR_N_C) < MAX_ANALOG_VALUE_DETECTOR_C)){
